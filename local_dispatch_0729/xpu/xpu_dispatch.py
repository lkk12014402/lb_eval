...
cp "$WORK"/_report.py                "$WORK/xpu/report.py"
mkdir -p "$WORK/xpu/agent/copilot_config"
cp "$WORK"/_agent_backends.sh        "$WORK/xpu/agent/agent_backends.sh"
cp "$WORK"/_agent_fix_loop.sh        "$WORK/xpu/agent/agent_fix_loop.sh"
cp "$WORK"/_agent_settings.json      "$WORK/xpu/agent/copilot_config/settings.json"
cp "$WORK"/_agent_autoround_pr.sh    "$WORK/xpu/agent/autoround_pr.sh"
cp "$WORK"/_agent_report_agent.sh    "$WORK/xpu/agent/report_agent.sh"
cp "$WORK"/_agent_patch_capture.sh   "$WORK/xpu/agent/patch_capture.sh"
...
def _collect_secrets() -> dict:
    src = {
        ...
        # Agent escalation ladder config (non-secret, but flows via the same env-file).
        # Default ladder lives in xpu_pipeline.sh; only forwarded when overridden.
...
def _staged_payload(run_id, req, script, secrets) -> dict:
    files = {
        ...
        "_report.py": ((_PARENT / "report.py").read_bytes(), 0o600),
        "_agent_backends.sh": ((_PARENT / "agent" / "agent_backends.sh").read_bytes(), 0o600),
        "_agent_fix_loop.sh": ((_PARENT / "agent" / "agent_fix_loop.sh").read_bytes(), 0o600),
        "_agent_settings.json": ((_PARENT / "agent" / "copilot_config" / "settings.json").read_bytes(), 0o600),
        "_agent_autoround_pr.sh": ((_PARENT / "agent" / "autoround_pr.sh").read_bytes(), 0o600),
        "_agent_report_agent.sh": ((_PARENT / "agent" / "report_agent.sh").read_bytes(), 0o600),
        "_agent_patch_capture.sh": ((_PARENT / "agent" / "patch_capture.sh").read_bytes(), 0o600),
