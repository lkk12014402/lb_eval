...
def select_machine_profile(server_kw: str, gpus_arg: str | None, need_gb: float) -> Candidate | None:
    """Select a static multi-hop profile, or return None for reservation-API machines."""
    try:
        from machine_profiles import get_profile
        profile = get_profile(server_kw)
    except ValueError:
        return None

    if profile.execution_mode not in ("nvidia_cuda", "login_only", "xpu_execution"):
        raise SystemExit(f"Unsupported execution mode for {profile.name}: {profile.execution_mode}")

    spec = spec_for_model("B200") if profile.hardware == "NVIDIA B200" else None
    if gpus_arg and not gpus_arg.startswith("auto"):
        gpu_ids = [int(x) for x in gpus_arg.split(",") if x.strip()]
    else:
        if gpus_arg and gpus_arg.startswith("auto"):
            count = int(gpus_arg.split(":", 1)[1]) if ":" in gpus_arg else 1
        else:
            count = min_cards_to_fit(spec, need_gb) if spec and need_gb > 0 else 1
            count = count or 1
        gpu_ids = list(range(count))
    invalid = [i for i in gpu_ids if i < 0 or i > 7]
    if invalid:
        raise SystemExit(f"GPU {invalid} 不存在；{profile.display_name} 可用编号: 0..7")
    eff = effective_vram(spec, len(gpu_ids)) if spec else 0.0
    return Candidate(
        server_id=f"profile:{profile.name}",
        server_name=profile.display_name,
        host=profile.target,
        model="B200" if profile.hardware == "NVIDIA B200" else profile.hardware,
        quality=spec.quality if spec else 0,
        n_cards=len(gpu_ids),
        gpu_ids=gpu_ids,
        effective_vram_gb=round(eff, 1),
        machine_profile=profile.name,
    )
...
def dispatch(model_id: str, scheme: str, user: str, hours: int, start: str | None,
             ...
             server: str | None = None, gpus: str | None = None,
    ...
    # Static multi-hop machines are outside the reservation API.
    profile_cand = select_machine_profile(server, gpus, need_gb) if manual else None
    if profile_cand is not None:
        cand = profile_cand
        result["manual"] = True
        result["machine_profile"] = cand.machine_profile
        ...
        result["start_time"] = "immediate"
        result["duration_hours"] = hours
        result["reservation"] = None
        result["reservation_id"] = None
        result["reservation_skipped"] = "static multi-hop machine"
        if dry_run:
            result["dry_run"] = True
            return result
        if no_ssh:
        ...
        ok, detail = verify_profile(cand.machine_profile)
        result["ssh_ok"] = ok
    ...
    # Apply host override (stale reservation-API IP → real IP)
    if host_override:
        print(f"[host-override] {cand.host} → {host_override}")
        cand.host = host_override
...
def main() -> int:
    ...
    p.add_argument("--host-override", default=None,
                   help="override the SSH/dispatch host IP (use when the reservation API "
                        "still lists a stale IP). Reservation still uses --server; only "
                        "the connection target is replaced. E.g. --host-override 10.239.11.179")
    ...
    p.add_argument("--results-dataset",
                   default=os.environ.get("LOCAL_RESULTS_DATASET", "lvkaokao/lb_local"),
                   help="HF dataset repo for local results (default: lvkaokao/lb_local)")
    ...
    if res.get("dry_run"):
        ...
        return 0
    if res.get("reservation_skipped"):
        print(f"Reserved: skipped ({res['reservation_skipped']})")
    elif res.get("reservation_error"):
...
def _run_on_xpu(args, cand: dict) -> int:
    """Hand off a B60 request to the standalone XPU dispatcher."""
    import os as _os, sys as _sys
    _xpu_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "xpu")
    if _xpu_dir not in _sys.path:
        _sys.path.insert(0, _xpu_dir)
    import xpu_dispatch
    ...
    print(f"\n\u2550\u2550\u2550\u2550\u2550\u2550 XPU execution on {cand['host']} "
          f"(cards {cand['gpu_ids']}) \u2550\u2550\u2550\u2550\u2550\u2550")
    res = xpu_dispatch.dispatch(
        model=args.model, scheme=args.scheme, method=args.method,
        export_format=args.export_format, gpu_ids=cand["gpu_ids"],
        remote_root=args.remote_root or xpu_dispatch.DEFAULT_REMOTE_ROOT,
        image=args.image or xpu_dispatch.DEFAULT_IMAGE,
        base_image=_os.environ.get("XPU_BASE_IMAGE", xpu_dispatch.DEFAULT_BASE_IMAGE),
        git_repo=args.git_repo or xpu_dispatch.DEFAULT_GIT_REPO,
        git_branch=args.git_branch or xpu_dispatch.DEFAULT_GIT_BRANCH,
        results_dataset=args.results_dataset or xpu_dispatch.DEFAULT_RESULTS_DATASET,
        request_json=req, no_openclaw=False, dry_run=args.run_dry,
    )
    if args.run_dry:
        print(res["script"])
        return 0
...
def _run_on_host(args, cand: dict) -> int:
    """Hand off to step 2 on the reserved machine (CUDA remote_exec or XPU dispatcher)."""
    if cand.get("machine_profile") == "b60-xpu":
        return _run_on_xpu(args, cand)
    import remote_exec
    ...
    if args.results_dataset: overrides["results_dataset"] = args.results_dataset
    ...
    job = remote_exec.RemoteJob(
        ...
        request_json=req, machine_profile=cand.get("machine_profile", ""), **overrides,
    )
    ...
    password = os.environ.get("LOCAL_SSH_PASS", "")
    if not password and not cand.get("machine_profile"):
