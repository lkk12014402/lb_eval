...
def gpu_df() -> tuple[pd.DataFrame, str]:
    ...
    if not rows:
        return pd.DataFrame(columns=["server", "host", "gpu", "total", "available", "busy"]), err
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Local Dispatch") as demo:
        ...
        with gr.Tab("🏆 Leaderboard"):
            gr.Markdown("Finished / failed runs (from `results/` in the lb_eval repo), "
        ...
        with gr.Tab("📋 Queue"):
            gr.Markdown("Submitted requests and their status (from `status/`).")
        ...
        with gr.Tab("📨 My Dispatches"):
            ...
            log_box = gr.Textbox(label="Log (live tail)", lines=24, max_lines=24,
                                 interactive=False, autoscroll=True)

            def _log_view(mode, job_id):
                if mode == "All running (combined)":
                    return runner.read_logs_combined(running_job_ids())
                return refresh_log(job_id)

            # Manual refresh button (also refreshes the dropdown choices).
            gr.Button("🔄 Refresh now").click(
                lambda m, j: (jobs_df(), gr.update(choices=job_choices()),
                              _log_view(m, j), running_summary()),
                inputs=[view_mode, job_sel], outputs=[jt, job_sel, log_box, run_summary])

            # Live auto-refresh: ticks every 2s while the checkbox is on.
            live = gr.Timer(2.0)

            def _tick(enabled, mode, job_id):
                if not enabled:
                    return gr.update(), gr.update(), gr.update()
                return jobs_df(), _log_view(mode, job_id), running_summary()
            live.tick(_tick, inputs=[auto_refresh, view_mode, job_sel],
        ...
        with gr.Tab("➕ Submit"):
            ...
            with gr.Row():
                server_in = gr.Textbox(label="--server (manual)", placeholder="L20x8-smc-1 / 118.195… / 4090D")
