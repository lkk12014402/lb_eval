...
Multi-hop NVIDIA B200 profiles are also execution-enabled:

- `sc09-b200`: Paramiko jump + `direct-tcpip` + target SFTP/streaming;
- `changwa1-b200`: jump-host OpenSSH alias/key + tar-over-SSH staging/streaming.

Both reuse the same per-run isolation, GPU pinning, Docker image, model upload and
independent HF dataset result upload. Both B200 profiles use direct network access
and ignore inherited Intel proxy variables. `b60-xpu` remains login-only.
...
### Local result upload (independent from the online leaderboard)

Local dispatch does **not** write `results/`, `status/`, or lessons to the online
lb_eval GitHub repository. The container runs `auto.sh --skip-upload`, then:

1. successful quantized weights still use lb_eval's `upload_model_hf.py` and go to
   a Hugging Face **model** repository;
2. logs, accuracy, reports, request JSON, recipes and agent sessions are uploaded by
   `upload_results_hf_dataset.py` to the Hugging Face **dataset**
   `lvkaokao/lb_local` (configurable with `LOCAL_RESULTS_DATASET`);
3. `quantized_model/` is excluded from the dataset to avoid duplicating weights;
4. failed-run analysis remains local and is included in the dataset, but is not
   pushed to the online leaderboard GitHub/community.

Dataset layout:

```text
results/<org>/<artifact>/run_<local_run_id>/...
results/<org>/<artifact>/results_<local_run_id>.json
status/<org>/<request_filename>.json
```

Token selection: `LOCAL_RESULTS_HF_TOKEN` first, otherwise the first `HF_TOKENS`
token is reused.
