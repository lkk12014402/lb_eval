...
**读哪个路径**：`datasrc.py` 的 `LB_EVAL_REPO` 决定，解析顺序：

1. 环境变量 **`LB_EVAL_UI_REPO`**（优先）
2. 默认 **`local_dispatch/../lb_eval`**（即 `/root/new_commit/lb_eval`）
...
- **不会 `git pull`**：UI 从头到尾只读本地磁盘那个 lb_eval 目录，不会去 GitHub 同步。
  显示的新旧完全取决于**那个本地目录本身有多新**。
- **会重新读盘**：每次点 “🔄 Refresh” 或重启 app.py，都会重新 `os.walk` 扫最新的本地文件。
  若该目录被别的进程更新了（你手动 `git pull`，或 auto.sh 回传用的就是同一目录），刷新即可见。

**让 UI 显示最新线上结果的两种办法**：

1. 指向一个会定期 `git pull` 的克隆：`export LB_EVAL_UI_REPO=/path/to/live/lb_eval_clone`
2. 让 UI 自己定期拉取 —— **当前未实现**（可按需增加后台定时 `git pull`）。

> 注意：默认的 `/root/new_commit/lb_eval` 是改代码用的**干净仓库**，不是线上实际回传的那个，
> 所以 UI 里看到的是这份的历史数据。
...
| 命名带 ModelFree | `remote_exec.py` / `upload_results_github.py` | 文件名如 `..._MXFP4_4bit_mxfp4_MODEL_FREE.json`，结果目录 `...-AutoRound-MXFP4-ModelFree` |
